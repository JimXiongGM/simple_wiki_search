...
# Allow: bash scripts/run_agent_baseline_random_300.sh MODEL_NAME=foo
for arg in "$@"; do
  case "${arg}" in
    *=*)
      export "${arg%%=*}=${arg#*=}"
    ...
    *)
      ...
      echo "Usage: bash scripts/run_agent_baseline_random_300.sh [KEY=VALUE ...]" >&2
      exit 1
      ;;
  esac
done
...
OUTPUT_ROOT="${OUTPUT_ROOT:-output_sample_random_300}"
PROVIDER="${PROVIDER:-}"
BASE_URL="${BASE_URL:-}"
API_KEY="${API_KEY:-}"
...
# bash scripts/run_agent_baseline_random_300.sh MODEL_NAME=google/gemma-4-12B-it
# bash scripts/run_agent_baseline_random_300.sh MODEL_NAME=mistralai/Ministral-3-8B-Instruct-2512-BF16
